...
def unieval_dialogue(contexts, conversation_histories, predictions):
    ...
    for i in range(len(predictions)):
        ...
        if conversation_histories is not None:
            cur['source'] = conversation_histories[i]
        if contexts is not None:
            cur['context'] = contexts[i]
        json_data.append(cur)
    # Initialize evaluator for a specific task
    evaluator = get_evaluator("dialogue")
    # Get multi-dimensional evaluation scores
    eval_scores = evaluator.evaluate(json_data, print_result=True)
    coherence = []
    naturalness = []
    engagingness = []
    groundedness = []
    ...
    overalls = []
    for eval_score in eval_scores:
        coherence.append(eval_score["coherence"])
        naturalness.append(eval_score["naturalness"])
        engagingness.append(eval_score["engagingness"])
        groundedness.append(eval_score["groundedness"])
        understandability.append(eval_score["understandability"])
        overalls.append(eval_score["overall"])
    results = {}
    results["coherence"] = np.mean(coherence)
    results["naturalness"] = np.mean(naturalness)
    results["engagingness"] = np.mean(engagingness)
    results["groundedness"] = np.mean(groundedness)
    results["understandability"] = np.mean(understandability)
    results["overall"] = np.mean(overalls)
    return results
...
def evaluating_summaries(gold_summaries, generated_summaries, source_documents=None):
    score_bert = bert_score(generated_summaries, gold_summaries)
    rouge_results = rouge_corpus(gold_summaries, generated_summaries)
    score_bart = bart_score(generated_summaries, gold_summaries)
    score_unieval = unieval(source_documents=source_documents, references=gold_summaries,
